...
class SpeechCorpus(object):
    ...
    def __init__(self, batch_size=16, set_name='train'):
        ...
        features_list = pickle.load(open("features_list.p","rb"))
        levels_list = pickle.load(open("levels_list.p","rb"))


        # to constant tensor
        features_list_t = tf.convert_to_tensor(features_list)
        levels_list_t = tf.convert_to_tensor(levels_list)

        # create queue from constant tensor
        levels_list_q, features_list_q = tf.train.slice_input_producer([features_list, levels_list], shuffle=True)
